...
// Sort by size
const bySize = [...entries].sort((a, b) => b.response.bodySize - a.response.bodySize);
console.log('\n--- Top 10 Largest Responses ---');
bySize.slice(0, 10).forEach(e => {
  console.log(`${(e.response.bodySize / 1024).toFixed(2)} KB - ${e.request.method} ${e.request.url.split('?')[0]}`);
});
